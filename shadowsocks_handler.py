...
from bot.ss_password_utils import generate_ss_password  # Предполагается, что функция доступна

logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)  # Отключаем DEBUG для производительности


class UserCache:
    """LRU кэш для последних активных пользователей"""
    ...
    def get(self, key: str) -> Optional[str]:
        """Получить username по ключу, обновляя LRU порядок"""
...
class ShadowsocksServer:
    """Shadowsocks AEAD сервер, использующий users.json"""
    ...
    def __init__(self, buffer_size=65536):
        ...
        self.max_attempts_per_ip = 10  # Максимум 10 неудачных попыток
        self.ban_duration = 300  # 5 минут бан

        # Оптимизация идентификации: кэш + предзагруженные пароли из JSON
        self.users_file = Path(__file__).parent / "users.json"  # Убедитесь, что файл рядом с этим скриптом
        self.user_cache = UserCache(max_size=100)  # Перемещено сюда
        self.cached_users: Dict[str, str] = {}  # username -> ss_password
        self.last_users_refresh = 0
        self.users_refresh_interval = 30  # Обновляем список каждые 30 секунд
        ...
        try:
            ...
            for user_id, user_info in users_data.items():
                ...
                if username and ss_password:
                    ...
                    logger.debug(f"[SS-CACHE] Loaded user: {username}")  # Для отладки
    ...
    async def handle_connection(self, reader, writer):
        ...
        if peer_ip in self.failed_attempts:
            ...
            if fail_count >= self.max_attempts_per_ip:
                ...
                if time_since_ban < self.ban_duration:
                    logger.warning(
                        f"[SS] IP {peer_ip} is temporarily banned ({self.ban_duration - time_since_ban:.0f}s remaining)")
                    writer.close()
                    return
                else:
        ...
        try:
            ...
            # Обновляем кэш пользователей перед идентификацией
            ...
            if not username:
                ...
                # Инкрементим счётчик неудачных попыток
                if peer_ip in self.failed_attempts:
            ...
            # Успешная идентификация — сбрасываем счётчик неудачных попыток
            if peer_ip in self.failed_attempts:
                del self.failed_attempts[peer_ip]

            # Получаем пароль из кэша (уже загруженного из JSON)
            ss_password = self.cached_users.get(username)
            if not ss_password:
                logger.warning(
                    f"[SS] User {username} found during identification but password missing from cache/json.")
                writer.close()
                return
            ...
            # Парсим адрес назначения и извлекаем первые данные
            addr_type = payload[0]
            header_len = 0
            ...
            # Извлекаем данные после адреса (если есть)
            ...
            # Проверка разрешений УДАЛЕНА - разрешаем подключаться куда угодно
            # is_allowed = (
            ...
            # Используем os.urandom из стандартной библиотеки
            ...
            # Отправляем начальные данные (если есть) на remote сервер
            if initial_data:
            ...
            # limiter = None # Убран rate limiter
            results = await asyncio.gather(
                self._pipe_decrypt(reader, remote_writer, aead, nonce_c2s, username, "c2s"),  # Убран limiter
                self._pipe_encrypt(remote_reader, writer, response_aead, nonce_s2c, username, "s2c"),  # Убран limiter
                return_exceptions=True
            )

            # Проверяем, были ли ошибки в tasks
            ...
            # if username:
            #     await self.connection_manager.disconnect(username, peer_ip, connection_id) # Убрана логика отключения
            writer.close()
    ...
    async def _refresh_users_cache(self):
        """Обновление списка пользователей из JSON (каждые 30 секунд)"""
        now = time.monotonic()
        if now - self.last_users_refresh < self.users_refresh_interval:
            return

        try:
            new_cache = self.load_users_from_json()  # Вызов функции загрузки из JSON
            self.cached_users = new_cache
            self.last_users_refresh = now
            logger.info(f"[SS] Refreshed user cache from JSON: {len(self.cached_users)} users")

        except Exception as e:
            logger.error(f"[SS] Failed to refresh users cache from JSON: {e}")

    async def _try_decrypt_and_identify(self, salt: bytes, length_chunk: bytes) -> Optional[str]:
        """
        ОПТИМИЗИРОВАННАЯ идентификация пользователя:
        1. Проверяем LRU кэш последних активных (O(1))
        2. Перебираем предзагруженный список паролей из памяти (из JSON)
        3. Обновляем кэш при успехе
        """
        # Шаг 1: Список пользователей уже должен быть загружен в self.cached_users вызывающим

        if not self.cached_users:
            logger.warning("[SS] No users in cache from JSON, cannot identify")
            return None

        # Шаг 2: Создаем ключ для LRU кэша (первые 16 байт salt)
        cache_key = hashlib.sha256(salt[:16]).hexdigest()[:16]

        # Шаг 3: Проверяем LRU кэш
        ...
        if cached_username and cached_username in self.cached_users:
            ...
            try:
                ...
                # Кэш устарел, продолжаем перебор
                pass

        # Шаг 4: Перебор всех пользователей из памяти (из JSON)
        attempts = 0
        max_attempts = 300
        ...
        for username, ss_password in self.cached_users.items():
            ...
            try:
                ...
                if 0 < length <= 0x3FFF:
                    ...
                    # Логируем статистику кэша каждые 100 успешных идентификаций
                ...
                # Быстрый fail - не логируем каждую неудачную попытку
                continue

        logger.warning(f"[SS] Failed to identify user after {attempts} attempts")
        return None

    async def _pipe_decrypt(self, reader, writer, aead, nonce, username, direction):  # Убран limiter
        """Читает зашифрованные данные, расшифровывает и отправляет открытым текстом (c2s)"""
        try:
            total_bytes = 0
            packet_count = 0
            while not reader.at_eof():
                # Читаем зашифрованную длину (2 байта + 16 tag)
                try:
                ...
                # Читаем зашифрованный payload (payload + 16 байт AEAD tag)
                ...
                logger.debug(
                    f"[SS] {direction} {username}: packet #{packet_count}, decrypted {len(data)} bytes (total: {total_bytes})")

                # await self.bandwidth_manager.track_usage(username, len(data)) # Убрано
                await asyncio.sleep(0)  # yield control

                # TCP backpressure вместо asyncio.sleep()
                # if limiter:
                #     wait_time = await limiter.consume(len(data))
                #     if wait_time > 0:
                #         # Отправляем маленькими чанками для создания TCP backpressure
                #         chunk_size = 8192  # 8KB чанки
                #         offset = 0
                #         while offset < len(data):
                #             chunk = data[offset:offset + chunk_size]
                #             writer.write(chunk)
                #             await writer.drain()  # TCP сам throttle'ит
                #             offset += chunk_size
                #     else:
                #         # Нет ограничения — отправляем всё сразу
                #         writer.write(data)
                #         await writer.drain()
                # else:
                ...
                # Отправляем всё сразу без лимитера
                writer.write(data)
                await writer.drain()
            ...
            # Нормальное закрытие соединения клиентом/сервером
    ...
    async def _pipe_encrypt(self, reader, writer, aead, nonce, username, direction):  # Убран limiter
        try:
            total_bytes = 0
            max_payload_size = 0x3FFF  # 16383 байт - максимум для Shadowsocks AEAD
            ...
            while not reader.at_eof():
                ...
                # Разбиваем большие чанки на части по max_payload_size
                offset = 0
                packets_to_send = []
                while offset < len(data):
                    chunk = data[offset:offset + max_payload_size]

                    # ПРАВИЛЬНЫЙ порядок: сначала длина, потом payload
                ...
                #     wait_time = await limiter.consume(len(data))
                #     if wait_time > 0:
                #         # Отправляем пакеты с throttling через drain()
                #         for packet in packets_to_send:
                #             writer.write(packet)
                #             await writer.drain()  # TCP backpressure
                #     else:
                #         # Нет ограничения — отправляем все пакеты сразу
                #         for packet in packets_to_send:
                #             writer.write(packet)
                #         await writer.drain()
                # else:
